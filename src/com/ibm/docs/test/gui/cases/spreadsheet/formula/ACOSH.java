 ...
 * 
 */
public class ACOSH extends BaseTest {
	@Drive
	...
	public void atGeneralIO() {
		...
		// test 1
		String input1 = GFormula.parse("=ACOSH(1)", localeID);
		String expected1 = GUtils.getFormattedNumber(0d, localeID, 0, false);
		...
		// test 2
		String input2 = GFormula.parse("=ACOSH(10)", localeID);
		String expected2 = GUtils.getFormattedNumber(2.993222846d, localeID, 9,
				false);
	...
	public void atReference() {
		String localeID = GUtils
				.getBrowserPreferenceLan(spreadsheetEditorPage.driver);
		String data = GUtils.getFormattedNumber(10d, localeID, 0, false);
		spreadsheetEditorPage.typeInCell("A1", "Data").typeInCell("A2", data)
				.typeInCell("A3", "Formula").typeInCell("B3", "Result");
		String expected = GUtils.getFormattedNumber(2.993222846d, localeID, 9,
				false);
		// 1.Verify cell reference.
		log.info("Verify cell reference.");
		String input1 = GFormula.parse("=ACOSH(A2)", localeID);
		...
		String input2 = GFormula.parse(
				"=ACOSH('" + spreadsheetEditorPage.getSheetName(2) + "'!B2)",
				localeID);
		...
		spreadsheetEditorPage.selectSheet(1);
		String input3 = GFormula.parse("=ACOSH('" + sheetName + "'!B2)",
				localeID);
		...
		spreadsheetEditorPage.selectSheet(1);
		String input6 = GFormula.parse("=ACOSH(" + sheetName + ")", localeID);
